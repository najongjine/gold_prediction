...
🤖 AI에게 코딩을 지시할 때 사용하는 프롬프트 예시
(위 기획서 내용과 함께 아래 프롬프트를 AI에게 전달하면 완벽한 코드를 얻을 수 있습니다.)

"위에 제공한 **'딥러닝(GRU) 기반 금값 30일 후방 예측 모델 구축 기획서'**를 완벽하게 반영하여 Python PyTorch 코드를 작성해 줘.

[구현 시 필수 주의사항]

스텝 5의 TimeSeriesSplit 교차 검증 루프 내에서는 반드시 모델(GRUModel)과 옵티마이저를 갱신(초기화)하여 데이터 누수를 막아줘.

교차 검증이 끝난 후, 실전 예측을 위해 전체 데이터(X, y)로 최종 모델을 새롭게 학습하는 루프를 반드시 분리해서 작성해 줘.

마지막 출력부는 최근 스무딩 가격, 30일 뒤 예측 상승/하락률(%), 예상 최종 가격($)이 직관적으로 보이게 print문을 꾸며줘.

코드에는 한글 주석을 풍부하게 달아서 설계 의도를 파악하기 쉽게 해줘."
